...
template <class x>
Arreglo<x>::Arreglo(unsigned int dim, int indef):indefinido(indef){
...
template <class x>
Arreglo<x>::Arreglo(const Arreglo &arre):indefinido(arre.indefinido){
	this->MAX = arre.MAX;
	this->arreglo = reservarMemoria(this->MAX);
	if (this->arreglo ==NULL)
		this->MAX = 0;
...
template <class x>
bool Arreglo<x>::pertenece(int X){
	int k=1;
